...
class Game:
    ...
    def _update(self):
        ...
        ep = []
        p.whip.update(p, self.enemies, self.particles, self.damage_numbers, self.sfx)
        p.orb.update(p, self.enemies, self.particles, self.damage_numbers, self.sfx)
        p.aura.update(p, self.enemies, self.particles, self.damage_numbers, self.sfx)
    ...
    def _draw_ui(self, surf):
        ...
        hp_text = self.font_sm.render(f"{T('hp')}  {p.hp}/{p.max_hp}", True, C_WHITE)
        ...
        xp_text = self.font_sm.render(f"{T('lv')} {p.level}  {T('xp')} {p.xp}/{p.xp_needed}", True, C_WHITE)
